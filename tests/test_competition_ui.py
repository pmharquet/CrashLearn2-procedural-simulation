"""Opt-in native UI journey: CRASHLEARN_GPU_TESTS=1 python -m pytest tests/test_competition_ui.py."""
...
class CompetitionUiTests(unittest.TestCase):
    def test_paddock_countdown_race_results_restart_return(self):
        import pygame
        import crashlearn_sim.app as crashlearn
        from crashlearn_sim.ui.menu import RaceMenu
        from crashlearn_sim.ui.race_view import CompetitionRenderer
        ...
        with tempfile.TemporaryDirectory(prefix="pilote extérieur ") as directory:
            output = Path(directory) / "screenshots"
            output.mkdir()
            pilot = Path(directory) / "agent.py"
            pilot.write_text("class Agent:\n    def predict(self,obs,info): return 3.,0.\n")
            state = dict(
                stage=0, menu=None, live=None, window=None, start=time.monotonic(), frames=0
            )
            ...
            def click(name):
                return pygame.event.Event(
                    pygame.MOUSEBUTTONDOWN, button=1, pos=state["buttons"][name].center
                )
            ...
            def events():
                ...
                if state["stage"] == 1:
                    menu = state["menu"]
                    for d in menu.drivers:
                        d.model = str(pilot)
                    menu.command("add")
                    menu.command("add")
                    menu.command("name")
                    menu.buffer = "Équipe Azur"
                    menu.commit_edit()
                    state["stage"] = 2
            ...
            with (
                patch.object(RaceWindow, "__init__", window_start),
                patch.object(LiveRace, "__init__", live_start),
                patch.object(RaceMenu, "draw", menu_render),
                patch.object(CompetitionRenderer, "draw", race_render),
                patch.object(pygame.event, "get", events),
                patch(
                    "sys.argv",
                    ["crashlearn.py", "--renderer", "gpu", "--window-size", "1280", "900"],
                ),
            ):
                crashlearn.main()
