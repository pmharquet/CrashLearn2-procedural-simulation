...
Run with: docker compose run --rm sim python -m pytest tests/test_contract.py
"""
...
# ---------------------------------------------------------------------------
def test_t03_zero_action_stays_on_spot():
    reset(1)
    simulation_step()
    ...
    assert abs(info["progress_delta"][0]) < 0.01, (
        f"progress_delta[{0}]={info['progress_delta'][0]} >= 0.01"
    )
...
# ---------------------------------------------------------------------------
def test_t06_wall_flag_surfaces():
...
# ---------------------------------------------------------------------------
def test_t08_friction_initial_then_drop():
    reset(1)
    info = get_step_info()
    ...
    for _ in range(3):
        simulation_step()
    info2 = get_step_info()
    assert info2["friction_current"] < 1.0, (
        f"friction should have dropped, got {info2['friction_current']}"
    )
...
# ---------------------------------------------------------------------------
def test_t09_lap_time_after_crossing():
    reset(1)
    sim_obj = _get_sim_raw()
    ...
    try:
        ...
        sim_obj._update_progress_and_laps(
            {"poses_x": [0.0] * sim_obj._num_agents, "poses_y": [0.0] * sim_obj._num_agents}
        )
    finally:
    ...
    assert bool(info["lap_complete"][0]) is True, (
        f"lap_complete must be True, got {info['lap_complete'][0]}"
    )
...
# ---------------------------------------------------------------------------
def test_t17_lap_booked_on_cum_crossing():
    reset(1)
    sim_obj = _get_sim_raw()
...
# ---------------------------------------------------------------------------
def test_t18_finish_at_required_laps():
    reset(1)
    sim_obj = _get_sim_raw()
...
# ---------------------------------------------------------------------------
def test_t19_two_laps_two_times():
    reset(1)
    sim_obj = _get_sim_raw()
    ...
    try:
        ...
        sim_obj._compute_progress = lambda x, y: (
            0.05
        )  # rel=(0.05-0)%1=0.05, d=(0.05-0.95)+1=0.1 => cum=2.05
        sim_obj._update_progress_and_laps(
            {"poses_x": [0.0] * sim_obj._num_agents, "poses_y": [0.0] * sim_obj._num_agents}
        )
    finally:
    ...
    assert len(sim_obj._lap_times[0]) >= 2, (
        f"only {len(sim_obj._lap_times[0])} lap times: {sim_obj._lap_times}"
    )
...
# ---------------------------------------------------------------------------
def test_t20_knockback_impulse_math():
...
def test_t25_no_dnf_after_lap():
    ...
    try:
        ...
        poses = {"poses_x": [0.0] * sim_obj._num_agents, "poses_y": [0.0] * sim_obj._num_agents}
...
# ---------------------------------------------------------------------------
def test_t24_lap_complete_guard():
    reset(1)
    sim_obj = _get_sim_raw()
