...
class RaceWindow:
    def __init__(self, size=None, fullscreen=False, backend='software', display=0):
        ...
        if self.gpu is None:
            self.screen = pygame.display.set_mode(self.windowed_size,pygame.RESIZABLE,display=self.display)
        if fullscreen:
    ...
    def toggle_fullscreen(self):
        if self.gpu is not None:
            ...
            size = pygame.display.get_desktop_sizes()[self.display] if fullscreen else self.windowed_size
    ...
    def _gpu_mode(self,size,fullscreen):
        from procedural_gpu import GpuScene
        if self.gpu is not None:
            self.gpu.release()
            self.gpu = None
        ...
        flags = pygame.OPENGL|pygame.DOUBLEBUF|(pygame.FULLSCREEN if fullscreen else pygame.RESIZABLE)
