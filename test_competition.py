...
from procedural_simulation import ProceduralSimulation, footprint_clear
from procedural_live import LiveRace
from procedural_recording import RaceReplay


def config(model='', track='', limit=2, count=2):
    return Competition(track=str(track), limit=limit, weather='original',
...
class CompetitionTests(unittest.TestCase):
    def test_procedural_matches_existing_simulation_exactly(self):
        ...
        for _ in range(35):
            for sim in (game,old): sim.step([(3.,.01),(3.,-.01)])
    ...
    def test_all_original_maps_close_and_grid_is_on_road(self):
        ...
        for path in tracks():
            ...
            self.assertGreater(sim.road.length,100.)
    ...
    def test_laps_unwrap_forward_backward_and_finish(self):
        ...
        for station,previous in ((.2,-.2),(length+.2,length-.2),(length-.2,length+.2),
                                 (2*length+.2,2*length-.2)):
            sim.distances[0]=previous
            x,y,yaw=sim.road.pose(station)
            sim.states[0,:2]=[x,y]; sim.states[0,4]=yaw
            self.assertAlmostEqual(sim.project_progress(0),station,places=5)
        sim.best[0]=2*length-.2
        sim.step((0.,0.))
    ...
    def test_finite_sectors_and_infinity(self):
        self.assertEqual(CompetitionSimulation(config(limit=7)).finish_distance,700.)
    ...
    def test_cancel_loading_terminates_hung_pilot(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            live=LiveRace(Namespace(competition=config(pilot,count=1),record=None,speed=1),wait=False)
            time.sleep(2)
