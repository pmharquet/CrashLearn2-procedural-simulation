...
from procedural_ui import RaceRenderer, TEXT, MUTED, ACCENT
from procedural_simulation import WEATHER_MODES, ROAD_PROFILES
from race_competition import Competition, Driver, CompetitionSimulation, CircuitRoad, tracks


def ui_state():
    return dict(follow=0, camera_yaw=0., overview=False, infos=True, lidar=False,
                trails=True, paused=False, speed=1, actual_speed=0., fps=0.,
                replay=False, recording=False, message='')
...
class CompetitionRenderer(RaceRenderer):
    def _draw_controls(self, frame, ui, width, height):
        ...
        if not frame.get('competition'): return
        ...
        if countdown is not None:
            ...
            for i in range(3):
                pygame.draw.circle(self.screen, ACCENT if countdown <= 3-i else (55,65,70),
                                   (width//2+(i-1)*38,height//2-115),10)
        elif ui['paused']:
...
class RaceMenu:
    ...
    def __init__(self):
        ...
        self.stage = 'track'
        default = Path(__file__).resolve().parent.parent / 'T-AIA-901-NCY-9-1-crashlearn-2/submission/best/agent.py'
        self.drivers = [Driver(f'Pilote {i+1}', PALETTE[i], str(default) if default.is_file() else '') for i in range(4)]
        self.count = 2
    ...
    def refresh_preview(self):
        ...
        if self.preview_key == key: return
        config = Competition(track=self.config.track, seed=self.config.seed,
                             profile=self.config.profile, weather='original', drivers=self.drivers[:1])
        ...
        self.preview_length = frame['sector_length'] if config.track else 100.
    ...
    def draw(self, screen, gpu=None, loading=False, result=None):
        ...
        if self.canvas.get_size()!=self.size or self.background is None:
            ...
            draw_table(self.background,project,(0,0),math.ceil(math.hypot(width,height)/56)+4)
        ...
        a.text('01  CIRCUIT',width-690,44,ACCENT if self.stage=='track' and result is None else MUTED)
        a.text('02  PADDOCK',width-505,44,ACCENT if self.stage=='garage' and result is None else MUTED)
        a.text('03  RESULTATS' if result else '03  EN PISTE',width-304,44,ACCENT if result else MUTED)
        ...
        if loading:
            ...
            for i in range(3):
                pygame.draw.circle(self.canvas,ACCENT if int(time.monotonic()*3)%3==i else MUTED,(x+55+i*28,y+141),6)
            a.buttons.clear()
        ...
        return {key:pygame.Rect(r.x*w/width,r.y*h/height,r.w*w/width,r.h*h/height)
                for key,r in a.buttons.items()}

    def draw_tracks(self):
        a = self.art
        width,height=self.size
        sidebar=min(720,400+max(0,width-1280)*.375)
        ...
        a.text('CIRCUIT ORIGINAL / EDITION RC' if self.config.track else 'UN NOUVEAU TRACE A CHAQUE GRAINE',44,176,MUTED,a.small)
        ...
        fit=min(viewport.w/max(1.,hi[0]-lo[0]),viewport.h/max(1.,hi[1]-lo[1]))
        def project(p):
        ...
        a.text(f'{self.preview_length:.0f} m / '+('tour' if self.config.track else 'secteur'),44,y+40,TEXT,a.big)
        a.text('OBJECTIF',right-490,y+16,MUTED,a.small)
        limit = str(self.config.limit) if self.config.limit else '∞'
        a.text(limit+' '+('TOURS' if self.config.track else 'SECTEURS'),right-490,y+42,ACCENT,a.big)
        ...
        a.button('weather','METEO  /  '+self.config.weather.upper(),(right+20,bottom-136,sidebar-40,34))
        if not self.config.track:
            half=(sidebar-50)/2
            a.button('seed',f'GRAINE {self.config.seed}  /  MODIFIER',(right+20,bottom-92,half,32),self.edit=='seed')
            a.button('profile',self.config.profile.upper(),(right+30+half,bottom-92,half,32))
    ...
    def draw_track_grid(self,viewport):
        ...
        for index,track in enumerate(self.track_list):
            tile=pygame.Rect(viewport.x+(index%columns)*(tile_width+gap),
                             viewport.y+(index//columns)*(tile_height+gap)-self.track_scroll,
                             tile_width,tile_height)
            visible=tile.clip(viewport)
            if not visible.w or not visible.h: continue
            active=index==self.track_index
            hover=visible.collidepoint(a._mouse)
            pygame.draw.rect(self.canvas,(38,54,66) if active or hover else (28,40,49),tile,border_radius=9)
            pygame.draw.rect(self.canvas,ACCENT if active else (59,72,81),tile,2 if active else 1,border_radius=9)
            ...
            if key not in self.thumbnail_points:
                if track:
                    ...
                    preview=CompetitionSimulation(Competition(seed=self.config.seed,profile=self.config.profile,
                                                              weather='original',drivers=self.drivers[:1])).road.center
            ...
            scale=min(bounds.w/max(1.,hi[0]-lo[0]),bounds.h/max(1.,hi[1]-lo[1]))
            ...
            a.text(self.shorten(title,tile.w-20),tile.x+10,tile.bottom-25,ACCENT if active else TEXT,a.small)
        ...
        thumb_y=viewport.y+(viewport.h-thumb_height)*self.track_scroll/max(1,self.scroll_max)
        pygame.draw.rect(self.canvas,ACCENT,(viewport.right+8,thumb_y,7,thumb_height),border_radius=3)

    def scroll_event(self,event,screen_size):
        if self.stage!='track': return False
        if event.type==pygame.MOUSEWHEEL:
            self.track_scroll=max(0,min(self.scroll_max,self.track_scroll-event.y*90))
            return True
        if event.type==pygame.MOUSEBUTTONUP and event.button==1:
            self.drag_scroll=False
        if not hasattr(event,'pos') or self.grid_scrollbar is None: return False
        pos=(event.pos[0]*self.size[0]/screen_size[0],event.pos[1]*self.size[1]/screen_size[1])
    ...
    def draw_garage(self):
        ...
        for i,driver in enumerate(self.drivers[:self.count]):
            ...
            if i==self.selected:
                pygame.draw.rect(self.canvas,driver.color,(x+1,228,card_width-2,363+extra),2,border_radius=14)
            ...
            pygame.draw.ellipse(self.canvas,(12,21,28),(x+card_width/2-91,479+extra/2,183,32))
            draw_car(self.canvas,(x+card_width/2,401+extra/2),driver.color,-16,226)
            a.text(self.shorten(driver.name,card_width-35,a.big),x+18,520+extra,driver.color,a.big)
            a.text(self.shorten(Path(driver.model).parent.name if driver.model else 'Aucun pilote sélectionné',card_width-36),x+18,560+extra,MUTED,a.small)
        ...
        a.button('name',self.buffer+'|' if self.edit=='name' else driver.name+'  /  RENOMMER',(40,660+extra,260,48),self.edit=='name')
        ...
        a.text(self.shorten(driver.model or 'agent.py + poids ONNX, ou modèle PPO (.zip)',width-696),653,630+extra,MUTED,a.small)
        a.button('browse','ASSOCIER UN PILOTE  /  EXPLORATEUR',(650,660+extra,width-690,48))
        ...
        a.text(f"{name}  /  {self.config.limit or '∞'} {'tours' if track else 'secteurs'}",222,height-49,TEXT)
    ...
    def text_event(self,event):
        if self.edit is None: return False
        if event.type==pygame.TEXTINPUT:
            text=event.text
            if self.edit != 'name': text=''.join(c for c in text if c.isascii() and c.isdigit())
            self.buffer=(self.buffer+text)[:24 if self.edit=='name' else 8]
        elif event.type==pygame.KEYDOWN:
            if event.key==pygame.K_BACKSPACE: self.buffer=self.buffer[:-1]
            elif event.key==pygame.K_ESCAPE: self.edit=None
            elif event.key in (pygame.K_RETURN,pygame.K_KP_ENTER): self.commit_edit()
        return True

    def commit_edit(self):
        if self.edit=='name':
            self.drivers[self.selected].name=self.buffer.strip() or f'Pilote {self.selected+1}'
        elif self.edit=='limit': self.config.limit=max(1,int(self.buffer or '1'))
        elif self.edit=='seed':
    ...
    def command(self,command):
        if self.edit: self.commit_edit()
        self.error=''
        if command.startswith('track_'):
            self.track_index=int(command.split('_')[1]); self.sync(); self.refresh_preview()
        elif command in ('track','garage'):
            self.stage=command
        elif command=='weather': self.config.weather=WEATHER_MODES[(WEATHER_MODES.index(self.config.weather)+1)%len(WEATHER_MODES)]
        elif command=='profile':
            self.config.profile=ROAD_PROFILES[(ROAD_PROFILES.index(self.config.profile)+1)%len(ROAD_PROFILES)]
            self.refresh_preview()
        elif command=='limit_up': self.config.limit=min(99999999,(self.config.limit or 0)+1)
        elif command=='limit_down': self.config.limit=max(1,(self.config.limit or 2)-1)
        elif command=='infinite': self.config.limit=None if self.config.limit else 3
        elif command in ('name','limit','seed'):
            self.edit=command
            self.buffer=self.drivers[self.selected].name if command=='name' else str(getattr(self.config,command) or '')
        ...
        elif command.startswith('driver_'): self.selected=int(command.split('_')[1])
        elif command.startswith('color_'): self.drivers[self.selected].color=PALETTE[int(command.split('_')[1])]
            ...
            try:
                if command=='browse':
                    path=filedialog.askopenfilename(parent=root,title='Sélectionner le pilote et son modèle',
                        filetypes=[('Pilotes IA','*.py *.onnx *.zip'),('Tous les fichiers','*.*')])
                    if path: self.drivers[self.selected].model=path
                else:
                    color,_=colorchooser.askcolor(parent=root,initialcolor='#%02x%02x%02x'%tuple(self.drivers[self.selected].color))
                    if color: self.drivers[self.selected].color=tuple(int(c) for c in color)
