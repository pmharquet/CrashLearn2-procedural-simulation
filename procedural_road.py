...
@njit(cache=True)
def integrate_profile(start,curvatures,spacing=.5):
    points = np.empty((len(curvatures),4))
    previous = start.copy()
    old_k = 0.
    for i,k in enumerate(curvatures):
        yaw = previous[2]+.5*(old_k+k)*spacing
...
def point_segment_distance2(px,py,ax,ay,bx,by):
    vx,vy = bx-ax,by-ay
    t = min(1.,max(0.,((px-ax)*vx+(py-ay)*vy)/(vx*vx+vy*vy)))
...
def segment_distance2(a,b,c,d):
    ...
    return min(point_segment_distance2(a[0],a[1],c[0],c[1],d[0],d[1]),
...
def tube_clear(points,first_new,clearance=5.05):
    ...
    for i in range(max(1,first_new),len(points)):
        ...
        for j in range(1,i):
            ...
            if a[3]-d[3] <= 8.: continue
            if (min(a[0],b[0])-max(c[0],d[0])>clearance or
                min(c[0],d[0])-max(a[0],b[0])>clearance or
                min(a[1],b[1])-max(c[1],d[1])>clearance or
                min(c[1],d[1])-max(a[1],b[1])>clearance): continue
            if segment_distance2(a,b,c,d)<clearance*clearance: return False
    return True


def curvature_profile(commands,spacing=.5):
    ...
    def flush():
        ...
        if len(group)==1:
            ...
            result.extend(target*np.asarray([shoulder]+[1.]*plateau+[shoulder,0.]))
            group.clear()
            return
        values = []
        previous = 0.
        for angle,radius in group:
            ...
            previous = target
        values.extend(np.linspace(previous,0.,3)[1:])
    ...
    for angle,value in commands:
        if angle == 0:
            flush()
            result.extend([0.]*max(1,int(np.ceil(value/spacing))))
        else:
...
class EndlessRoad:
    ...
    behind = 50.
    planning_ahead = 180.
    names = ('ligne droite','courbe ouverte','angle droit','double coude','chicane',
             'triple chicane','esses','epingle 180','double epingle','boucle ouverte')

    def __init__(self,seed=0,profile='mixed'):
        if profile not in ('mixed','flowing','technical'): raise ValueError('Unknown road profile')
        self.seed = seed
        self.profile = profile
        self.rng = np.random.default_rng(seed)
        self._plan = [np.array([s,0.,0.,s]) for s in np.arange(-50.,8.5,.5)]
        self.generated = 0
        self.removed = 0
    ...
    def commands(self,name,sign):
        ...
        if name == 'ligne droite': return [(0,float(rng.uniform(12,36)))]
        if name == 'courbe ouverte': return [(sign*float(rng.uniform(25,70)),float(rng.uniform(10,20)))]
        if name == 'angle droit': return [(sign*90.,float(rng.uniform(4.2,7.5))),(0,float(rng.uniform(3,8)))]
        if name == 'double coude': return [(sign*float(rng.uniform(80,105)),r),(-sign*float(rng.uniform(100,135)),r),(sign*float(rng.uniform(65,95)),r)]
        if name == 'chicane':
        ...
        if name == 'triple chicane':
            ...
            return [(sign*a,r*float(rng.uniform(1,1.15))) for a in (angle,-2*angle,2*angle,-2*angle,angle)]
        if name == 'esses': return [(sign*a,float(rng.uniform(5,8))) for a in (75,-135,120,-60)]
        if name == 'epingle 180':
            radius = float(rng.uniform(2.55,3.1) if rng.random()<.5 else rng.uniform(3.1,6.))
            return [(sign*180.,radius),(0,float(rng.uniform(4,9)))]
        if name == 'double epingle':
            r = float(rng.uniform(2.55,5.8))
            return [(sign*180.,r),(0,float(rng.uniform(5,9))),(-sign*180.,r),(0,4.)]
        if name == 'boucle ouverte':
            ...
            return [(sign*angle,radius),(-sign*float(rng.uniform(angle-170,angle-120)),
                                        radius*float(rng.uniform(.6,1.))),(0,4.)]
    ...
    def _choose(self):
        ...
        weights = []
        for name in choices:
            weight = .55 if name in ('epingle 180','boucle ouverte') else .3 if name == 'double epingle' else 1.
            if self.profile == 'flowing':
                weight = 3. if name in ('ligne droite','courbe ouverte') else .15 if name in ('epingle 180','double epingle','boucle ouverte') else .6
            elif self.profile == 'technical':
                weight = .6 if name in ('ligne droite','courbe ouverte') else .8 if name in ('epingle 180','double epingle','boucle ouverte') else 2.
    ...
    def _append_section(self):
        ...
        for attempt in range(120):
            ...
            if attempt >= 80:
                name = 'raccord'
                commands = [(sign*float(self.rng.uniform(25,150)),float(self.rng.uniform(3.8,7.)))]
            ...
            if not tube_clear(combined,len(existing),self.width+.25):
                self.rejected += 1
                continue
            ...
            self.sections.append(dict(name=name,start=start,end=float(candidate[-1,3]),
                                      turns=[float(a) for a,_ in commands if a],
                                      yaw_start=float(existing[-1,2]),yaw_end=float(candidate[-1,2])))
            ...
            if not self._append_section():
                ...
                if self.sections and self.sections[-1]['start']>published_end and repairs<32:
                    ...
                    for _ in range(depth):
                        if not self.sections or self.sections[-1]['start']<=published_end: break
                    ...
                    repairs += 1
                    continue
                raise RuntimeError(f'No collision-free continuation: seed={self.seed}, s={self._plan[-1][3]:.1f}')
    ...
    def _trim(self,cutoff):
        cut = 0
        while cut+1<len(self._plan) and self._plan[cut+1][3]<cutoff: cut+=1
        if cut:
            self._plan = self._plan[cut:]
            self.removed += cut
        while self.sections and self.sections[0]['end']<self._plan[0][3]: self.sections.popleft()

    def project(self,pos):
        # Deferred import avoids an import cycle with the shared physics module.
        from procedural_simulation import project_center
