...
import pygame
import moderngl
from procedural_art import car_model
...
class GpuScene:
    def __init__(self):
        ...
        self.textured = self.ctx.program(vertex_shader=TEXTURE_VERTEX,fragment_shader=TEXTURE_FRAGMENT)
        ...
        self.quad = self.ctx.vertex_array(self.textured,[(self.quad_buffer,'2f 2f','position','uv')])
    ...
    def triangles(self,points,color):
        ...
        data[:,:2] = points
        data[:,2:] = np.array((*color[:3],color[3] if len(color)>3 else 255))/255.
        self.parts.append(data)

    def strip(self,left,right,color):
        left,right = np.asarray(left),np.asarray(right)
        self.triangles(np.stack((left[:-1],right[:-1],left[1:],
                                 right[:-1],right[1:],left[1:]),axis=1),color)
    ...
    def lines(self,points,color,width=1):
        ...
        if len(points)>1: self.segments(points[:-1],points[1:],color,width)

    def circle(self,center,radius,color):
        angles = np.linspace(0,2*np.pi,41)
        self.lines(np.asarray(center)+radius*np.column_stack((np.cos(angles),np.sin(angles))),color,1.2)

    def flush(self):
        if self.parts:
            data = np.concatenate(self.parts)
            if data.nbytes > self.buffer.size: self.buffer.orphan(data.nbytes)
            self.buffer.write(data)
            self.mesh.render(moderngl.TRIANGLES,vertices=len(data))
            self.parts.clear()

    def texture(self,surface):
        native = surface.get_pitch()==surface.get_width()*4 and surface.get_masks()==(16711680,65280,255,4278190080)
    ...
    def draw(self,frame,ui,trails,colors,size,patches,rain_field,impacts):
        ...
        for program in (self.solid,self.textured,self.wood): program['size'].value = size
        ...
        anchor = np.array([width*.53,height*.64])
        if ui['overview']:
            ...
            origin = (lo+hi)/2
            scale = min((width-2*margin)/max(1.,hi[0]-lo[0]),(height-235*factor)/max(1.,hi[1]-lo[1]))
            anchor = np.array([width/2,(height+5*factor)/2])
        def project(points):
            p = np.asarray(points)-origin
            if ui['overview']: return p*[scale,-scale]+anchor
            return np.column_stack((-s*p[:,0]+c*p[:,1],-c*p[:,0]-s*p[:,1]))*scale+anchor
        ...
        self.quad_buffer.write(np.array([[0,0,0,0],[width,0,1,0],[0,height,0,1],[width,height,1,1]],dtype='f4'))
        ...
        outer_l,outer_r = project(road+normals*(half+.38)),project(road-normals*(half+.38))
        ...
        self.lines(left,(231,229,204),max(2,scale*.15))
        self.lines(right,(231,229,204),max(2,scale*.15))
        if patches:
        ...
        self.segments(left[:-1][marked],left[1:][marked],(232,83,57),max(3,scale*.19))
        self.segments(right[:-1][marked],right[1:][marked],(232,83,57),max(3,scale*.19))
        ...
        sector_labels = [(f'S{sectors[i+1]}',cp[i]+[10,0]) for i in np.flatnonzero(marked)
                         if 0<=cp[i,0]<width and 0<=cp[i,1]<height]
        self.lines([left[0],right[0]],(222,99,114),4)
        self.lines([left[-1],right[-1]],(247,192,88),4)
        for i,trail in enumerate(trails):
            if ui.get('trails',True) and len(trail)>1: self.lines(project(trail),tuple(int(x*.6) for x in colors[i]),2)
        if ui['lidar']:
            from procedural_simulation import LIDAR_ANGLES
            angles = LIDAR_ANGLES+states[follow,4]
            pos = states[follow,:2]
            ends = pos+np.asarray(frame['lidar'][follow])[:,None]*np.column_stack((np.cos(angles),np.sin(angles)))
            starts = np.repeat(project([pos]),len(ends),axis=0)
            ends = project(ends)
            near = np.asarray(frame['lidar'][follow])<=.11
        ...
        for i,car in enumerate(states):
            if frame['status'][i] != 1: continue
            dot = project([car[:2]])[0]
            angle = np.pi/2-car[4] if ui['overview'] else car[4]-yaw
            length = max(24.,min(78.,scale*1.5))
            if i == follow: self.circle(dot,length*.57,colors[i])
            if rain>.2:
                tail = project([car[:2]-.7*np.array([np.cos(car[4]),np.sin(car[4])])])[0]
            ...
            if i in impacts or frame['wall_hits'][i] or frame['vehicle_hits'][i]:
                ...
                angles = np.arange(8)*np.pi/4+frame['time']*.5
                rays = np.column_stack((np.cos(angles),np.sin(angles)))
                self.segments(dot+rays*(17+age*13),dot+rays*(29+age*23),(255,204,105,int(255*(1-age))),2.5)
                self.segments(dot+rays*(18+age*13),dot+rays*(23+age*18),(255,248,211,int(255*(1-age))),1.3)
            self.flush()
            ...
            self.label(str(i+1),dot+[length*.4,-8],color)
        for text,position in sector_labels: self.label(text,position,(232,206,129))
        if rain>.05:
            self.triangles([[0,0],[width,0],[0,height],[width,0],[width,height],[0,height]],(64,85,111,int(25*rain)))
            starts,ends,brightness = rain_field.segments(frame['time'],size,rain)
            self.segments(starts,ends,(110,158,189))
            if rain>.7 and frame['time']%13<.1:
                self.triangles([[0,0],[width,0],[0,height],[width,0],[width,height],[0,height]],(185,210,231,24))
            self.flush()

    def overlay(self,surface,update=True):
        if self.hud is None or self.hud.size != surface.get_size():
            if self.hud is not None: self.hud.release()
    ...
    def screenshot(self,path):
        ...
        image = pygame.image.frombytes(self.ctx.screen.read(viewport=(0,0,*size),components=3),size,'RGB')
        pygame.image.save(pygame.transform.flip(image,False,True),str(path))

    def release(self):
        for item in (*self.textures.values(),*self.labels.values(),self.hud,self.mesh,self.quad,self.wood_quad,
                     self.buffer,self.quad_buffer,self.solid,self.textured,self.wood):
            if item is not None: item.release()
        self.ctx.release()
