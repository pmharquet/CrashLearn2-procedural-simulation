"""Desktop competition launcher; the existing demo/training CLIs stay available."""
import argparse
...
def main():
    ...
    from race_menu import RaceMenu, CompetitionRenderer, ui_state
    ...
    parser.add_argument('--frames',type=int,default=0,help='Quitter après N images (vérification).')
    ...
    try:
        ...
        if args.verify_agent:
            from race_verification import verify_distribution
            if not verify_distribution(window,args.verify_agent,args.verify_output,args.screenshot):
                raise SystemExit(1)
            return
        ...
        result=None
        last_recording=None
        recording_dir=Path(os.environ.get('LOCALAPPDATA',str(Path.home()))) / 'CrashLearn' / 'recordings'
        countdown=0.
        loading_since=0.
        replay_index=0
        replay_elapsed=0.
        ...
        def launch():
            nonlocal live,phase,loading_since,result,ui,renderer,previous_ticks,previous_calls,measured
            menu.sync()
            ...
            live=LiveRace(Namespace(competition=menu.config,record=None,speed=1,recording_dir=recording_dir),wait=False)
            phase='loading'
        ...
        while running:
            elapsed=min(.15,clock.tick(120)/1000)
            commands=[]
            for event in pygame.event.get():
                if event.type==pygame.QUIT: running=False
                elif event.type in (pygame.VIDEORESIZE,pygame.WINDOWSIZECHANGED):
                    window.resized(); renderer.screen=window.screen
                    ...
                    window.toggle_fullscreen(); renderer.screen=window.screen; renderer.gpu=window.gpu
                elif phase=='menu' and menu.edit and event.type in (pygame.TEXTINPUT,pygame.KEYDOWN):
                    ...
                    if event.key==pygame.K_ESCAPE:
                        commands.append('cancel' if phase=='loading' else 'menu' if phase in ('race','countdown','replay') else 'track')
                    elif phase in ('race','countdown','replay'):
                        mapping={pygame.K_SPACE:'pause',pygame.K_TAB:'overview',pygame.K_l:'lidar',
                                 pygame.K_t:'trails',pygame.K_h:'infos',pygame.K_c:'follow',pygame.K_w:'weather',
                                 pygame.K_e:'record',pygame.K_v:'replay',pygame.K_r:'start',
                                 pygame.K_PLUS:'faster',pygame.K_EQUALS:'faster',pygame.K_KP_PLUS:'faster',
                                 pygame.K_UP:'faster',pygame.K_MINUS:'slower',pygame.K_KP_MINUS:'slower',pygame.K_DOWN:'slower'}
                        if event.key in mapping: commands.append(mapping[event.key])
                        elif pygame.K_1<=event.key<=pygame.K_9: ui['speed']=event.key-pygame.K_0
                        elif event.key==pygame.K_0: ui['speed']=10
                        elif replay and event.key in (pygame.K_LEFT,pygame.K_RIGHT,pygame.K_HOME,pygame.K_END):
                            delta=200 if event.mod & pygame.KMOD_SHIFT else 20
                            replay_index=(0 if event.key==pygame.K_HOME else replay.count-1 if event.key==pygame.K_END else
                                max(0,min(replay.count-1,replay_index+(delta if event.key==pygame.K_RIGHT else -delta))))
                            frame=replay.frame(replay_index); replay_elapsed=0.
            for command in commands:
                try:
                    if command=='quit': running=False
                    elif command in ('cancel','menu'):
                        close_replay(); close_live(); phase='menu'; menu.stage='garage'; result=None
                    elif command=='start':
                        if menu.edit: menu.commit_edit()
                        launch()
                    elif phase in ('menu','results'):
                        if phase=='results' and command=='track': phase='menu'; result=None
                        ...
                        if command in ('pause','overview','lidar','trails','infos'):
                            ...
                            if replay:
                                close_replay(); phase='race'; ui['paused']=True
                                renderer.colors=[tuple(d.color) for d in menu.config.drivers]
                                ...
                                if last_recording:
                                    ...
                                    replay_index=0; replay_elapsed=0.
                                    frame=replay.frame(0); ui['replay']=True; ui['paused']=False; ui['follow']=0
                                    renderer.colors=[tuple(c) for c in frame.get('colors',PALETTE[:len(frame['states'])])]
                                    phase='replay'
                                else: ui['message']='Aucun enregistrement : appuyer sur E'
                        elif command in ('weather','record') and not replay: live.send(command)
                except Exception as error:
                    if phase in ('menu','results','loading'):
                        close_live(); phase='menu'; menu.error=str(error)
                    else: ui['message']=str(error)

            if phase=='loading':
                try:
                    packet=live.poll()
                    if packet:
                        frame=packet['frame']; phase='countdown'; countdown=3.
                        ui['camera_yaw']=frame['states'][0][4]
                    elif time.monotonic()-loading_since>120: raise TimeoutError('Chargement trop long : vérifiez les pilotes.')
                except Exception as error:
                    close_live(); phase='menu'; menu.error=str(error).splitlines()[-1]

            if phase in ('race','countdown','replay'):
                try:
                    if phase=='countdown':
                        if not ui['paused']: countdown-=elapsed
                        if countdown<=0: phase='race'
                    elif countdown>-.7: countdown-=elapsed
                    ui['countdown']=countdown if countdown>-.7 else None
                    if phase=='replay':
                        ui['countdown']=None
                        if not ui['paused']:
                            ...
                            while replay_elapsed>=replay.dt:
                                ...
                                if replay_index==replay.count-1: ui['paused']=True; break
                        frame=replay.frame(replay_index)
                        ui['recording']=False
                    else:
                        live.set_pace(ui['speed'],ui['paused'] or phase=='countdown')
                        packet=live.poll(); frame=live.visual_frame()
                        ...
                        if measured>=.5:
                            ...
                            ui['actual_speed']=ui['ticks_per_second']*.05
                            previous_ticks=packet['ticks']; previous_calls=packet['ai_calls']; measured=0.
                        if frame['terminated'] or frame['truncated']:
                            result=dict(frame,colors=renderer.colors.copy())
                            close_live(); phase='results'
                    ...
                    ui['camera_yaw']+=((yaw-ui['camera_yaw']+np.pi)%(2*np.pi)-np.pi)*min(1.,elapsed*4*ui['speed'])
                    ui['fps']=clock.get_fps()
                except Exception as error:
                    close_replay(); close_live(); phase='menu'; menu.error=str(error).splitlines()[-1]

            if phase in ('menu','loading','results'):
                buttons=menu.draw(window.screen,window.gpu,phase=='loading',result if phase=='results' else None)
            else:
                renderer.draw(frame,ui); buttons=renderer.buttons
            frames+=1
            if args.screenshot and args.frames and frames>=args.frames:
                args.screenshot.parent.mkdir(parents=True,exist_ok=True)
                if window.gpu: window.gpu.screenshot(args.screenshot)
                else: pygame.image.save(window.screen,str(args.screenshot))
            pygame.display.flip()
