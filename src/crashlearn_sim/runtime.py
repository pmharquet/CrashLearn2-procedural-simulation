...
def _run(args, commands, frames, stop=None):
    ...
    pilots = None
    try:
        from crashlearn_sim.agents import make_agents, drive
        from crashlearn_sim.simulation.world import (
            ProceduralSimulation,
            WEATHER_MODES,
            ROAD_PROFILES,
        )
        from crashlearn_sim.recording import RaceRecorder

        competition = getattr(args, "competition", None)
        if competition:
            from crashlearn_sim.competition.models import CompetitionSimulation
        ...
        def publish():
            packet = dict(
                ...
                recording=bool(recorder),
                last_recording=str(last_recording) if last_recording else None,
                message=message,
            )
        ...
        while stop is None or not stop.is_set():
            ...
            while commands.poll():
                ...
                if command == "pace":
                    ...
                    sim.set_weather(
                        WEATHER_MODES[
                            (WEATHER_MODES.index(sim.weather_mode) + 1) % len(WEATHER_MODES)
                        ]
                    )
                    sim._refresh_noise()
                elif command in ("reset", "profile"):
                    if competition:
                        continue  # The launcher owns race transitions.
                    if command == "profile":
                        sim.profile = ROAD_PROFILES[
                            (ROAD_PROFILES.index(sim.profile) + 1) % len(ROAD_PROFILES)
                        ]
                    sim.reset(sim.seed + 1)
                    accumulator = restart = 0.0
                    if recorder:
                        recorder.append(sim.snapshot())
                    ...
                    if recorder:
                        ...
                        last_recording = Path(getattr(args, "recording_dir", "recordings")) / (
                            "race-" + datetime.now().strftime("%Y%m%d-%H%M%S-%f") + ".sqlite"
                        )
                    ...
                    commands.send(
                        dict(last_recording=str(last_recording) if last_recording else None)
                    )
            ...
            if not paused:
                if sim.terminated or sim.truncated:
                    ...
                    accumulator = min(accumulator + elapsed * speed, 0.6)
            ...
            delay = min(next_publish - now, 0.002 if not paused else 0.01)
            if delay > 0:
                time.sleep(delay)
...
class LiveRace:
    def __init__(self, args, wait=True):
        ...
        self.process = ctx.Process(
            target=_run,
            args=(args, child, self.frames, self.stop),
    ...
    def poll(self):
        ...
        while True:
            try:
                ...
                if self.packet is None or (packet["frame"]["seed"], packet["frame"]["step"]) != (
                    self.packet["frame"]["seed"],
                    self.packet["frame"]["step"],
                ):
    ...
    def visual_frame(self):
        if self.previous is None or (self.pace and self.pace[1]):
            return self.packet["frame"]
        interval = max(0.001, self.packet["stamp"] - self.previous["stamp"])
    ...
    def stop_recording(self):
        ...
        self.send("barrier")
        if not self.commands.poll(10):
            raise TimeoutError("Recorder did not close")
