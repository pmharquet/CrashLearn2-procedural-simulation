...
def tracks():
    ...
    return sorted(
        {p.name: p for p in MAPS.glob("*/*_centerline.csv")}.values(),
...
def agent_file(selection):
    ...
    if path.suffix.lower() != ".py" or not path.is_file():
        raise ValueError(
            "Le modèle ONNX doit être accompagné de son agent.py (prétraitement et actions)."
        )
    return path
...
class CircuitRoad:
    ...
    def __init__(self, path):
        ...
        self.center = np.column_stack(
            (np.vstack((points, points[0])), np.r_[yaw, closing_yaw], np.r_[stations, self.length])
        )
    ...
    def pose(self, station):
        station %= self.length
        return np.array(
            [np.interp(station, self.center[:, 3], self.center[:, j]) for j in range(3)]
        )
...
class CompetitionSimulation(ProceduralSimulation):
    ...
    def reset(self, seed=0):
        ...
        if self.num_cars > 1:
            self.states[:, 1] = [(-0.85 if i % 2 == 0 else 0.85) for i in range(self.num_cars)]
        if self.circuit:
            ...
            for i in range(self.num_cars):
                ...
                lateral = (-0.85 if i % 2 == 0 else 0.85) if self.num_cars > 1 else 0.0
    ...
    def snapshot(self):
        ...
        return dict(
            frame,
            names=names,
            colors=[d.color for d in self.config.drivers],
            events=events,
            competition=True,
            unit="TOUR" if self.circuit else "SECTEUR",
            track_name=Path(self.config.track).stem.replace("_centerline", "")
            if self.circuit
            else "Procédural",
