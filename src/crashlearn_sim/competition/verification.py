...
def verify_distribution(window, model, output, screenshot=None):
    ...
    from crashlearn_sim.competition.models import Competition, Driver, tracks
    ...
    try:
        ...
        while live.poll()["ticks"] < 20:
            ...
            if time.monotonic() > deadline:
                raise TimeoutError("Le pilote ne fait pas avancer la course.")
            if live.packet["frame"]["terminated"]:
                raise RuntimeError("Les pilotes ont abandonné avant 20 pas.")
            time.sleep(0.01)
        ...
        if screenshot:
            ...
            if window.gpu:
                window.gpu.screenshot(screenshot)
            else:
                pygame.image.save(window.screen, str(screenshot))
        pygame.display.flip()
        report = dict(
            ok=True,
            ticks=live.packet["ticks"],
            status=frame["status"],
