"""
Historical Crash&Learn Grand Prix simulation backend.

Private _Sim singleton wrapping f110_gym.envs.base_classes.Simulator directly
(NOT F110Env, NOT gym.make). Provides the historical module-level API.
...
# ---------------------------------------------------------------------------
# Installed map resources
# ---------------------------------------------------------------------------
import os
import warnings

from crashlearn_sim.paths import RESOURCES

_REPO_ROOT = str(RESOURCES)
...
def _ensure_maps_discovered() -> set[str]:
    ...
    if _available_maps is None:
        ...
        _available_maps = {
            ...
            if os.path.isdir(os.path.join(maps_root, d)) and d not in ("examples", "__pycache__")
        }
...
class _Sim:
    ...
    def __init__(self) -> None:
        ...
        self._last_delta: np.ndarray = np.zeros(
            _MAX_SLOTS
        )  # signed wrap-aware per-step d (for get_step_info)
        ...
        self._lap_start_step: np.ndarray = np.zeros(
            _MAX_SLOTS, dtype=int
        )  # step the current lap began
        self._finish_step: np.ndarray = np.full(
            _MAX_SLOTS, -1, dtype=int
        )  # step of FINISH (-1 if not)
        self._progress_history: list = [
            collections.deque(maxlen=_DNF_WINDOW_STEPS + 1) for _ in range(_MAX_SLOTS)
        ]
    ...
    def _jit_warmup(self) -> None:
        ...
        _project_progress_njit(0.5, 0.0, dummy_traj, np.array([0.0, 1.0]), 1.0)
...
def get_space_info() -> dict:
    ...
    return {
        "observations": {
            ...
            "steering": {
                "shape": "scalar",
                "bounds": (_PARAMS["s_min"], _PARAMS["s_max"]),
                "unit": "rad",
            },
            ...
            "opponents": {
                "shape": "dict[0..3]",
                "unit": "dict of {rel_x, rel_y, rel_dist, rel_yaw, velocity, active}",
            },
        },
        "actions": {
            "target_speed": {
                "bounds": (_TARGET_SPEED_MIN, _TARGET_SPEED_MAX),
                "unit": "m/s target velocity (asymmetric)",
            },
...
def set_map(name: str, num_cars: int | None = None) -> None:
    ...
    _MAP_EXT = ".png"
    _WPT_PATH = (
        os.path.join(base_dir, f"{config_base.replace('_map', '_centerline')}.csv")
        if name != "example"
        else os.path.join(_EXAMPLES_DIR, "example_waypoints.csv")
    )
                ...
                for line in _f:
                    ...
                    if len(parts) >= 2:
                        ...
                        with open(centerline_path) as _f2:
                            wpts_raw = []
                            for waypoint_line in _f2:
                                s = waypoint_line.strip()
