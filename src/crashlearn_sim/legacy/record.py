...
def main():
    """Run the historical record command."""
    import argparse
    import glob
    import math
    import os
    ...
    parser.add_argument(
        "--laps", type=int, default=2, help="Target number of laps to complete per car (default: 1)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to a submission directory or parent directory with team subfolders",
    )
    parser.add_argument(
        "--controller",
        type=str,
        default="auto",
        choices=["auto", "submission", "random", "onnx", "pure_pursuit"],
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Directory for saved episodes (default: /app/recordings or recordings/)",
    )
    parser.add_argument(
        "--out",
        type=str,
        default=None,
        help="Output path (default: <recordings-dir>/episode_XXX.npz)",
    )
    ...
    from crashlearn_sim.legacy.environment import _PARAMS as _EP
    ...
    from crashlearn_sim.agents import discover_and_load_submissions
    ...
    if sub_path:
        try:
            ...
            if len(loaded) == 1 and num_cars > 1:
                ...
                print(
                    f"Loaded submission '{single_name}' — replicated across {num_cars} cars (self-play)"
                )
    ...
    for step_i in range(T):
        ...
        if prev_obs is not None:
            for cid, obs in enumerate(prev_obs):
                ...
                if cid < len(team_agents) and team_agents[cid][1] is not None:
                    try:
                        spd, steer = team_agents[cid][1].predict(
                            obs, prev_info if prev_info is not None else {}
                        )
                    except Exception as e:
                        print(
                            f"[ERROR] Agent '{team_names[cid]}' (Car {cid}) predict failed at step {step_i}: {e}"
                        )
        ...
        if actual_steps % 100 == 0:
            ...
            print(
                f"step={actual_steps:4d}  sim_t={actual_steps * _SIM_DT:.1f}s  "
            ...
            if cid in finished_cars:
                ...
                results.append(
                    (
                        ...
                        cid,
                        tname,
                        f"FINISHED ({f_step * _SIM_DT:.2f}s)",
                        obs_list[cid]["lap_count"],
                    )
                )
            else:
                results.append(
                    (
                        1,
                        -float(max_prog[actual_steps - 1, cid]),
                        cid,
                        tname,
                        "DNF",
                        obs_list[cid]["lap_count"],
