...
import unittest
import numpy as np
from procedural_live import LiveRace, interpolate_frame


class LiveTests(unittest.TestCase):
    def test_interpolation_wraps_yaw_without_mutating_snapshots(self):
        a = dict(seed=1,step=1,status=[1],states=[[0,0,0,0,np.pi-.1,0,0]])
        b = dict(a,step=2,states=[[2,4,0,0,-np.pi+.1,0,0]])
        middle = interpolate_frame(a,b,.5)
        ...
        self.assertIs(interpolate_frame(a,dict(b,seed=2),.5)['states'],b['states'])
    ...
    def test_worker_records_every_tick_and_counts_each_agent(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            (root/'agent.py').write_text('class Agent:\n    def predict(self,obs,info): return 3.,0.\n')
            (root/'model.onnx').touch()
            path = root/'race.sqlite'
            args = Namespace(seed=42,cars=2,weather='original',profile='mixed',finish_distance=None,
                             agent=root,ppo=None,record=path,speed=10)
            live = LiveRace(args)
            try:
                ...
                try:
                    ...
                    for i in range(with_replay.count):
                        ...
                        if i+1<with_replay.count: reference.step([[3.,0.],[3.,0.]])
...
class NativeGpuTests(unittest.TestCase):
    def test_resize_fullscreen_orientation_and_collision(self):
        ...
        ui = dict(follow=0,camera_yaw=.4,overview=False,lidar=True,paused=False,speed=10,
                  actual_speed=10.,fps=120.,ticks_per_second=200.,ai_per_second=800.,
                  replay=False,recording=False,message='',infos=True)
        frame['states'][1][4] = 1.1
        frame['states'][1][:2] = [2.,2.]
        try:
            native = Window.from_display_module()
            for size in ((1600,1000),(800,600),(480,800),(1280,900)):
                native.size = size
                pygame.event.pump()
                with patch.object(pygame.display,'set_mode',wraps=pygame.display.set_mode) as recreate:
                ...
                for overview in (False,True):
                    ...
                    expected = np.pi/2-1.1 if overview else 1.1-.4
                    self.assertAlmostEqual(angle,expected)
                    self.assertTrue(all(renderer.screen.get_rect().contains(rect) for rect in renderer.buttons.values()))
                    pygame.display.flip()
            old_size = window.windowed_size
            renderer.screen = window.toggle_fullscreen(); renderer.gpu = window.gpu
            renderer.draw(frame,ui); pygame.display.flip()
            renderer.screen = window.toggle_fullscreen(); renderer.gpu = window.gpu
            ...
            with patch.object(window.gpu,'segments',wraps=window.gpu.segments) as segments:
                renderer.draw(frame,ui)
            self.assertTrue(any(call.args[2][:3]==(255,204,105) for call in segments.call_args_list))
            Path('logs').mkdir(exist_ok=True)
            window.gpu.screenshot('logs/gpu-collision-qa.png')
        finally:
            window.gpu.release()
            pygame.quit()
