...
PALETTE = ((35, 174, 244), (255, 177, 36), (244, 78, 61),
           (151, 211, 52), (172, 114, 242), (239, 239, 222))
...
def car_model(color):
    ...
    dark = tuple(int(c*.55) for c in color)
    light = tuple(int(c+(255-c)*.55) for c in color)
...
def rotated_car(color,heading,length):
    ...
    return pygame.transform.rotozoom(model,heading,length/260.)


def draw_car(screen, center, color, heading=0., length=64):
...
def draw_table(screen, project, position, radius=180):
    ...
    for row in range(math.floor((oy-radius)/9), math.ceil((oy+radius)/9)):
        y = row*9
        shade = (row*17)%13
        points = project([(ox-radius,y),(ox+radius,y),(ox+radius,y+9),(ox-radius,y+9)])
        ...
        pygame.draw.line(screen,(182,119,79),*project([(ox-radius,y+.16),(ox+radius,y+.16)]),1)
        for grain in range(1,9):
            pts = [(x,y+grain+.11*math.sin(x*.32+row+grain))
                   for x in range(math.floor(ox/4)*4-radius,math.floor(ox/4)*4+radius+4,4)]
